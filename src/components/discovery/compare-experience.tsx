...
const ROWS: CompareRow[] = [
  ...
  { id: "availability", label: "Availability", value: (p) => AVAILABILITY_LABEL[p.availability] },
  {
    ...
    label: "Special tags",
    value: (p) =>
      p.specialTags.length ? p.specialTags.map((tag) => SPECIAL_TAG_LABEL[tag]).join(", ") : "None listed",
  },
...
const STICKY_CELL =
  "sticky left-0 w-[9.5rem] min-w-[9.5rem] bg-[color:var(--surface-raised)] text-left align-top sm:w-52 sm:min-w-52";
  ...
  return (
    <Section tone="darker" space="none" className="pb-24 pt-28">
      <Container width="wide">
        <Eyebrow withRule>Side by side</Eyebrow>
        <div className="mt-5 flex flex-wrap items-end justify-between gap-6">
          <h1 className="text-display-lg">Compare Properties</h1>
          {selected.length > 0 ? (
            <button
              type="button"
        ...
        {!ready ? (
          ...
          <>
            ...
            <div className="mt-8 max-h-[calc(100dvh-9rem)] overflow-auto rounded-panel border border-[color:var(--hairline)] bg-[color:var(--surface-raised)]">
                ...
                <thead>
                  <tr>
                    <th
                      ...
                      className={cn(
                        STICKY_CELL,
                        "top-0 z-40 border-b border-r border-[color:var(--hairline)] px-4 py-4",
                      )}
                    >
                      <span className="eyebrow text-[color:var(--text-muted)]">Parameter</span>
                    </th>
                    {selected.map((project) => (
                      <th
                        key={project.slug}
                        scope="col"
                        className="sticky top-0 z-30 min-w-56 border-b border-[color:var(--hairline)] bg-[color:var(--surface-raised)] px-4 py-4 text-left align-top"
                      >
                        <span className="relative block aspect-[16/9] overflow-hidden rounded-xl bg-[color:var(--surface-sunken)]">
                          <Image
                        ...
                        <Link
                          ...
                          className="mt-3 block font-display text-base font-medium text-[color:var(--text-primary)] hover:text-[color:var(--accent)]"
                        >
                          {project.name}
                        </Link>
                    ...
                    return (
                      <tr key={row.id} className="border-b border-[color:var(--hairline)]">
                        <th
                          ...
                          className={cn(
                            STICKY_CELL,
                            "z-20 border-r border-[color:var(--hairline)] px-4 py-4 text-xs font-medium text-[color:var(--text-secondary)]",
                          )}
                        >
                          {row.label}
                          {differs ? (
                    ...
                    {selected.map((project) => (
                      <td key={project.slug} className="min-w-56 px-4 py-4 align-top">
                        <div className="flex flex-col gap-2">
                          <Button href={enquiryHref(project.slug)} size="sm">
                            Request Details
                          </Button>
