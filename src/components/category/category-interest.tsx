...
export function CategoryInterest({ category }: { category: Category }) {
    ...
    >
      ...
      <Container width="wide">
        ...
        <RevealGroup
          ...
          delay={0.2}
          className="mx-auto mt-14 grid max-w-4xl gap-px overflow-hidden rounded-panel border border-[color:var(--hairline)] bg-[color:var(--hairline)] sm:mt-16 sm:grid-cols-3"
        >
          {PROMPTS.map((prompt, index) => (
            <RevealItem key={prompt.title} mode="fade" className="bg-[color:var(--surface-raised)] p-6 sm:p-7">
              <span className={cn("eyebrow tabular", accentText(category.accent))}>
                {String(index + 1).padStart(2, "0")}
              </span>
              <h3 className="mt-3 font-display text-lg font-normal text-[color:var(--text-primary)]">
                {prompt.title}
              </h3>
